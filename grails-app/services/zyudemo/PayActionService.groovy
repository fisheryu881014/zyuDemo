...
class PayActionService {
    ...
    def payAction(String body, String total_fee, String userKey, String userLicense, String callbackUrl, def service) throws Exception {
        ...
        saveRecord(body, total_fee, payType, out_trade_no, client)
        String api_url = service.getApiUrl();
        callApiToPay(api_url, body, total_fee, out_trade_no, callbackUrl)   // 调用支付接口
    }

    private static void saveRecord(String body, String fee, PayType payType, String out_trade_no, Client client) {
        new PayRecord(body:body, fee: fee, payType: payType, tradeNo: out_trade_no, client: client).save()
    }
}
